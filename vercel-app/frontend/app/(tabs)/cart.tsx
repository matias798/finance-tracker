...
export default function CartScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const itemsRes = await fetch(`${API_URL}/api/items?type=cart`);
  ...
  const markAsPurchased = async (paidByUserId: string) => {
    ...
    try {
      await fetch(
        `${API_URL}/api/items/${selectedItem.id}/move-to-expense?paid_by=${paidByUserId}`,
        { method: 'PUT' }
      );
...
const styles = StyleSheet.create({
  ...
  summaryCard: { backgroundColor: '#1a1a2e', margin: 20, marginTop: 10, padding: 20, borderRadius: 16, alignItems: 'center' },
  summaryLabel: { color: '#888', fontSize: 14 },
  summaryAmount: { color: '#6366f1', fontSize: 36, fontWeight: 'bold', marginTop: 5 },
  summaryCount: { color: '#666', fontSize: 14, marginTop: 5 },
  listContent: { padding: 20, paddingTop: 0 },
  itemCard: { backgroundColor: '#1a1a2e', borderRadius: 12, padding: 16, marginBottom: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  itemInfo: { flex: 1 },
  ...
  itemActions: { flexDirection: 'row', marginTop: 8, gap: 10 },
  actionBtn: { padding: 4 },
  ...
  modalSubtitle: { color: '#888', fontSize: 16, marginBottom: 20 },
  ...
  userSelectButtons: { flexDirection: 'row', gap: 15, justifyContent: 'center' },
  userSelectBtn: { backgroundColor: '#6366f1', padding: 20, borderRadius: 12, alignItems: 'center', minWidth: 120 },
  userSelectName: { color: '#fff', fontSize: 16, fontWeight: '600', marginTop: 8 },
});
